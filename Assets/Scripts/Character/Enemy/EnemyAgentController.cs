...
public class EnemyAgentController : Agent
{
    ...
    private int min_time_until_next_bullet = 12000;
    ...
    public override void OnActionReceived(ActionBuffers actions)
    {
        ...
        isGrounded = enemyMovement.isGrounded;
        if (isGrounded && velocity.y < 0)
        {
            velocity.y = -2f;
        }

        Vector3 move = transform.forward * move_Forward;
        enemyMovement.Move(move * moveSpeed * Time.deltaTime);

        transform.Rotate(0f, move_Rotate, 0f, Space.Self);

        velocity.y += gravity * Time.deltaTime;
        enemyMovement.Move(velocity * Time.deltaTime);
